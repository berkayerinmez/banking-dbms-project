...
DROP FUNCTION IF EXISTS fn_set_account_branch();
...
-- Enforce at most one employee with position='MANAGER' per branch
CREATE UNIQUE INDEX uq_one_manager_per_branch
ON employee(branch_id)
WHERE position = 'MANAGER';

-- =========================
-- Manager table (for clearer ERD and explicit enforcement)
-- One manager record per branch
-- =========================
...
-- account.branch_id is required and must match customer.branch_id
-- =========================
...
-- Trigger to enforce account.branch_id always equals customer.branch_id
CREATE OR REPLACE FUNCTION fn_set_account_branch()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  v_customer_branch int;
BEGIN
  SELECT branch_id INTO v_customer_branch
  FROM customer
  ...
  -- Always set branch_id from customer to prevent inconsistency
  NEW.branch_id := v_customer_branch;

  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_set_account_branch ON account;

CREATE TRIGGER trg_set_account_branch
BEFORE INSERT OR UPDATE OF customer_id ON account
FOR EACH ROW
EXECUTE FUNCTION fn_set_account_branch();
...
-- =========================
-- Performance indexes for joins and dashboards
-- =========================
...
CREATE INDEX idx_manager_employee_id ON manager(employee_id);
